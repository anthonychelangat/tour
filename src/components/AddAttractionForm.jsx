...
import { addAttraction } from "@/lib/data";
...
const AddAttractionForm = () => {
  ...
  const [state, formAction] = useFormState(addAttraction, initialState);

  return (
    <form className="space-y-4 flex flex-col" action={formAction}>
      <div>
        <p className="font-bold mb-1">Attraction Name</p>
        <input
      ...
      <button
        ...
        name="submit"
        value="insert">
        {pending ? "Adding Attraction..." : "Add Attraction"}
      </button>
      <div>
        {state?.message ? (
